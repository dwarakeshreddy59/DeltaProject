      ...
      <div className="calc-body">
          ...
          {TDS_OPTIONS.map((o) => (
            <button
              ...
              style={{
                ...
                fontSize: ".82rem",
                transition: "all .2s",
                background: tds_rate === o.value ? "#312e81" : "#fff",
                borderColor: tds_rate === o.value ? "#312e81" : "#e2e8f0",
                color: tds_rate === o.value ? "#fff" : "#374151",
                boxShadow: tds_rate === o.value ? "0 4px 12px rgba(49,46,129,.3)" : "none",
              }}
            >
              {o.label}
            </button>
        ...
        <div className="calc-breakdown mt-3">
          <div className="breakdown-row">
            ...
            <span className="br-val" style={{ color: "#1565c0" }}>+ {fmt(assessable_value)}</span>
          </div>
          <div className="breakdown-row">
            <span className="br-label">GST ({gst_rate}%)</span>
            <span className="br-val" style={{ color: "#14b8a6" }}>+ {fmt(gst_amount)}</span>
          </div>
          <div className="breakdown-row">
            <span className="br-label">TDS ({tds_rate}%)</span>
            <span className="br-val" style={{ color: "#ef4444" }}>− {fmt(tds_amount)}</span>
          </div>
          <div className="breakdown-row" style={{ paddingTop: ".5rem", borderTop: "2px solid #e2e8f0", marginTop: ".25rem" }}>
            <span className="br-label" style={{ fontWeight: 700, color: "#0f172a" }}>Net Receivable</span>
            <span className="br-val" style={{ color: "#10b981", fontSize: "1rem" }}>{fmt(receivable)}</span>
          </div>
        </div>
      </div>
    </div>
